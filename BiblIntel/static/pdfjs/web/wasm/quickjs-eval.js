...
async function QuickJS(moduleArg = {}) {
  ...
  var la = (a, b, c, d) => {
    ...
    if (d)
      for (var l = 0; l < d.length; l++) {
        var Y = g[c[l]];
        Y ? (0 === p && (p = W()), (f[l] = Y(d[l]))) : (f[l] = d[l]);
      }
    c = a(...f);
    return (c = (function (k) {
      0 !== p && ka(p);
      return "string" === b ? Q(k) : "boolean" === b ? !!k : k;
    })(c));
  };
  U = (a, b, c) => {
    a = Q(a);
    b = null !== b ? JSON.parse(Q(b)) : [];
    try {
  ...
  e.cwrap = (a, b, c, d) => {
    ...
    return "string" !== b && g && !d ? e["_" + a] : (...f) => la(a, b, c, f, d);
  };
  e.stringToNewUTF8 = T;
  var S,
    ma,
    ...
    na = {
      ...
      c: (a, b, c, d) => {
        ...
        D[a >> 2] = 60 * Math.max(f, g);
        C[b >> 2] = Number(f != g);
        b = (p) => {
          var l = Math.abs(p);
          return `UTC${0 <= p ? "-" : "+"}${String(Math.floor(l / 60)).padStart(
            2,
            "0",
